# WildCam Sorter v1.6.4 — 红外相机照片与视频分类器 / Trail-camera Photo & Video Sorter
...
> 这是历史版本 **v1.6.4** 的说明。一般用户请优先下载 [最新版本 v1.10](https://github.com/S-Y-Chu/wildcam_sorter/releases/tag/v1.10)。

### 简介

v1.6 的稳定性维护版，重点修复视频解码、资源释放和界面可靠性。

WildCam Sorter 是一款本地运行的红外/野外相机媒体分类工具。它按组预览照片和视频，把选中的文件复制到物种类别目录，并保存进度和 CSV。原始输入文件不会被移动或改写。

### 本版本主要功能

- 输入与输出目录分离，分类只复制、不移动原始文件。
- 多类别和每文件独立分类。
- 自动识别部分已分类媒体，支持断点续传与重新分类。
- 改进视频后端回退、错误日志和长期运行稳定性。
...
按文件名自然排序，以检测到的视频为分组边界，支持 1 个视频 + 任意数量照片。
...
独立查看器提供缩放、拖动、适合窗口、1:1 与视频暂停。

### CSV、进度与日志

CSV 仍沿用 v1.6 的旧格式；如需每个文件一行和文件名列，请使用 v1.7 或更高版本。
...
- 打包：PyInstaller
- 源码分支：[v1.6.4](https://github.com/S-Y-Chu/wildcam_sorter/tree/1.6.4)
- 作者：Siyuan ZHU
...
> This document describes historical release **v1.6.4**. Most users should download the [latest release, v1.10](https://github.com/S-Y-Chu/wildcam_sorter/releases/tag/v1.10).

### Overview

A v1.6 maintenance release focused on video decoding, resource cleanup, and UI reliability.
...
- Separate input and output folders; sorting copies files and never moves the originals.
- Multi-category and independent per-file sorting.
- Detects some previously sorted media and supports resume and reclassification.
- Improved video-backend fallback, error logging, and long-session stability.
...
Natural filename ordering uses detected videos as group boundaries and supports one video plus any number of photos.
...
The standalone viewer provides zoom, drag, fit-to-window, 1:1, and video pause controls.

### CSV, progress, and logs

CSV retains the v1.6 legacy format. Use v1.7 or newer for one row per file and a filename column.
...
- Source branch: [v1.6.4](https://github.com/S-Y-Chu/wildcam_sorter/tree/1.6.4)
- Author: Siyuan ZHU
