...
- **媒体填充修复**：显示窗格变大时会按窗格实际尺寸重新加载预览；原文件分辨率很低时也会等比放大至窗格可用范围，不裁切照片。5 个媒体采用左侧半屏大格、右侧四小格，让横向视频在大格中确实显示得更大。
...
- **Actual display area:** When a grid cell grows, its preview reloads to the real cell size. Native low-resolution media is scaled to fit without cropping (it may look soft). Five-media groups use a half-width large video cell and a 2×2 set of smaller cells beside it, making the video visibly larger.
